...
class Dataset(ABC):
    ...
    def split_train_valid_test(self, ratios: tuple, random_state: int = 100):
        ...
        if summed != 1.0:
            ...
            if self.cache: # you want there to be a cache
                ...
                la = ["train", "valid", "test"]
                filenames = [os.path.join(CACHE_DIR, f"raw_{i}_split.pkl") for i in la]
                ...
                if cache_exists: # and you had the cache!
                    logger.info("Found and loading train, valid, test cached split...")
                    train = pickle.load(open(filenames[0], "rb"))
                    valid = pickle.load(open(filenames[1], "rb"))
                    test = pickle.load(open(filenames[2], "rb"))
                    return train, valid, test

            if not cache_exists: # there is no cache 
                logger.info("Found no cache, creating new split...")
                df = self.data
                ...
                if self.cache: # but you wanted there to be cache. so make cache.
                    logger.info("Saving the new split to cache.")
                    ...
                    for zipp in zipped:
                        with open(os.path.join(CACHE_DIR, f"raw_{zipp[1]}_split.pkl"), "wb") as f:
